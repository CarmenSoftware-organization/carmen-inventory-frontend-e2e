...
const NAME = fakeName({ tag: "EC" });
const NAME_UPDATED = fakeName({ tag: "EC Upd" });
...
test.describe("Extra Cost — Smoke & CRUD", () => {
  ...
  test(
    ...
    {
      annotation: [
        { type: "preconditions", description: "Login เป็น carmensoftware.dev+admin@gmail.com; อยู่ที่ /config/extra-cost" },
        { type: "steps", description: "1. ไปที่ /config/extra-cost" },
  ...
  test(
    ...
    async ({ page }) => {
      const h = new DialogCrudHelper(page, opts);
      const name = fakeName({ tag: "EC043" });
      const renamed = fakeName({ tag: "EC043 Upd" });
      await h.list.goto();
      await h.openAddDialog();
      await h.nameInput().fill(name);
      await h.saveButton().click();
