...
test.describe("Unit — Smoke", () => {
  test("TC-UN01 หน้า list โหลดสำเร็จ", async ({ page }) => {
    const list = new ConfigListPage(page, PATH);
    await list.goto();
    await expect(page).toHaveURL(new RegExp(PATH));
  });

  test("TC-UN02 ปุ่ม Add แสดง", async ({ page }) => {
    const list = new ConfigListPage(page, PATH);
    await list.goto();
    await expect(list.addButton()).toBeVisible();
  });

  test("TC-UN03 ช่องค้นหาใช้งานได้", async ({ page }) => {
    const list = new ConfigListPage(page, PATH);
    await list.goto();
  ...
  test("TC-UN04 ค้นหาคำที่ไม่มีต้องแสดง empty state", async ({ page }) => {
    const list = new ConfigListPage(page, PATH);
    await list.goto();
