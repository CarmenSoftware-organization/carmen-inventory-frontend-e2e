...
test.describe("Credit Note Reason — Smoke & CRUD", () => {
  test("TC-CNR01 หน้า list โหลดสำเร็จ", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await expect(page).toHaveURL(new RegExp(PATH));
  });

  test("TC-CNR02 ปุ่ม Add แสดง", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await expect(h.list.addButton()).toBeVisible();
  });

  test("TC-CNR03 ช่องค้นหาใช้งานได้", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
  ...
  test("TC-CNR04 ค้นหาคำที่ไม่มีต้องแสดง empty state", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
  ...
  test("TC-CNR05 บันทึกโดยไม่กรอกชื่อต้องแสดง error", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
  ...
  test("TC-CNR06 สร้างรายการใหม่และปรากฏในตาราง", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.openAddDialog();
  ...
  test("TC-CNR07 แก้ไขชื่อและบันทึก", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.list.search(NAME);
  ...
  test("TC-CNR13 แก้ไข: clear name แล้วบันทึก ต้องแสดง error", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.list.search(NAME_UPDATED);
  ...
  test("TC-CNR08 ลบรายการ", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.list.search(NAME_UPDATED);
