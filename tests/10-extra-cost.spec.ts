...
test.describe("Extra Cost — Smoke & CRUD", () => {
  test("TC-EC01 หน้า list โหลดสำเร็จ", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await expect(page).toHaveURL(new RegExp(PATH));
  });

  test("TC-EC02 ปุ่ม Add แสดง", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await expect(h.list.addButton()).toBeVisible();
  });

  test("TC-EC03 ช่องค้นหาใช้งานได้", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
  ...
  test("TC-EC04 ค้นหาคำที่ไม่มีต้องแสดง empty state", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
  ...
  test("TC-EC05 บันทึกโดยไม่กรอกชื่อต้องแสดง error", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
  ...
  test("TC-EC06 สร้างรายการใหม่และปรากฏในตาราง", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.openAddDialog();
  ...
  test("TC-EC07 แก้ไขชื่อและบันทึก", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.list.search(NAME);
  ...
  test("TC-EC13 แก้ไข: clear name แล้วบันทึก ต้องแสดง error", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.list.search(NAME_UPDATED);
  ...
  test("TC-EC08 ลบรายการ", async ({ page }) => {
    const h = new DialogCrudHelper(page, opts);
    await h.list.goto();
    await h.list.search(NAME_UPDATED);
