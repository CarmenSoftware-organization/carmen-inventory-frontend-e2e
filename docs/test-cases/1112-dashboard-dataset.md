...
> โมดูลนี้เป็น catalog แบบอ่านอย่างเดียว (read-only) ของ dashboard datasets ที่ระบบมีให้. ไม่มี create/edit/delete. รายการถูก group ตาม `category` และค้นหาได้ตาม id / name / description / category. แต่ละ DatasetCard แสดง name, badge `shape`, description, id และ unit. หัวข้อมี badge นับ total. มีสถานะ loading / error / empty (รวม empty หลัง filter).
