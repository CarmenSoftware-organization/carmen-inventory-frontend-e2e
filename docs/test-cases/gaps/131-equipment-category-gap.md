# Equipment Category — Gap Report

_เคสที่ **สเปกอัตโนมัติยังไม่ครอบ** เท่านั้น ไม่ใช่แคตตาล็อกเต็มของโมดูล — เคสที่เหลือถูกทดสอบจริงอยู่แล้วใน `tests/131-equipment-category.spec.ts` และมีเอกสารที่ generate ไว้ที่ `docs/user-stories/131-equipment-category.md`_

**Module:** Equipment Category
**Frontend route:** `routes/operation-plan/equipment-category`  •  **URL:** `/operation-plan/equipment-category`
**Prefix:** `EQPC`
**Spec ที่ครอบส่วนที่เหลือ:** `tests/131-equipment-category.spec.ts`
**Total test cases:** 4

> หมายเหตุสำคัญสำหรับผู้รีวิว: ไฟล์นี้แปลงมาจาก `docs/test-cases/131-equipment-category.md` เมื่อ 2026-09-20 แคตตาล็อกเดิมถือ TC-ID ชุดเดียวกับสเปกที่มีอยู่แล้ว ทำให้เอกสารสองชุดเป็นเจ้าของเลขเดียวกัน (audit ฟ้องเป็น CROSS_FILE_DUPLICATE) จึงเก็บไว้เฉพาะเคสที่สเปกยังไม่ครอบ เมื่อเคสใดถูกเขียนเป็นสเปกแล้วให้ลบออกจากไฟล์นี้ และเมื่อหมดทุกเคสให้ลบไฟล์ทิ้ง
