...
| --- | --- | --- | --- |
| TC-EC01 | หน้า list โหลดสำเร็จ | High | Smoke |
| TC-EC02 | ปุ่ม Add แสดง | High | Smoke |
| TC-EC03 | ช่องค้นหาใช้งานได้ | Medium | Smoke |
| TC-EC04 | ค้นหาคำที่ไม่มีต้องแสดง empty state | Medium | Functional |
| TC-EC05 | บันทึกโดยไม่กรอกชื่อต้องแสดง error | High | Validation |
| TC-EC06 | สร้างรายการใหม่และปรากฏในตาราง | High | CRUD |
| TC-EC07 | แก้ไขชื่อและบันทึก | High | CRUD |
| TC-EC08 | ลบรายการ | High | CRUD |
| TC-EC13 | แก้ไข: clear name แล้วบันทึก ต้องแสดง error | Medium | Validation |
| TCS-EC09 | XSS payload ในชื่อต้องไม่รัน script | High | Security |
| TCS-EC10 | SQL injection payload ต้องไม่ทำให้ระบบ crash | High | Security |
| TCS-EC11 | ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100 | Medium | Validation |
| TCS-EC12 _(skipped)_ | user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect | High | Authorization |
...
TC-EC06 ผ่านแล้ว → record NAME มีอยู่ใน DB
...
TC-EC13 ผ่านแล้ว → record NAME_UPDATED ยังคงมีอยู่ใน DB
...
TC-EC07 ผ่านแล้ว → record มี name = NAME_UPDATED
...
## TCS-EC09 — XSS payload ในชื่อต้องไม่รัน script
...
## TCS-EC10 — SQL injection payload ต้องไม่ทำให้ระบบ crash
...
## TCS-EC11 — ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100
...
## TCS-EC12 — user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect _(skipped)_
