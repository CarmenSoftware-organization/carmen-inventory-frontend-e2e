...
_Generated from `tests/011-location.spec.ts` annotations. Edit annotations, not this file. Regenerate with `bun docs:user-stories`._

**Module:** Location
**Spec:** `tests/011-location.spec.ts`
...
| --- | --- | --- | --- |
| TC-LOC01 | หน้า list โหลดสำเร็จ | High | Smoke |
| TC-LOC02 | ปุ่ม Add แสดง | High | Smoke |
| TC-LOC03 | ช่องค้นหาใช้งานได้ | Medium | Smoke |
| TC-LOC04 | ค้นหาคำที่ไม่มีต้องแสดง empty state | Medium | Functional |
| TC-LOC05 | บันทึกโดยไม่กรอก code/name ต้องแสดง error | High | Validation |
| TC-LOC06 | สร้างรายการใหม่และปรากฏในตาราง | High | CRUD |
| TC-LOC07 | แก้ไขชื่อและบันทึก | High | CRUD |
| TC-LOC08 | ลบรายการ | High | CRUD |
| TC-LOC13 | แก้ไข: clear code/name แล้วบันทึก ต้องแสดง error | Medium | Validation |
| TC-LOC14 | สร้าง location_type = Direct และลบ | Medium | CRUD |
| TC-LOC15 | สร้าง location_type = Consignment และลบ | Medium | CRUD |
| TCS-LOC09 | XSS payload ในชื่อต้องไม่รัน script | High | Security |
| TCS-LOC10 | SQL injection payload ต้องไม่ทำให้ระบบ crash | High | Security |
| TCS-LOC11 | ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100 | Medium | Validation |
| TCS-LOC12 _(skipped)_ | user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect | High | Authorization |
...
TC-LOC06 ผ่านแล้ว → record CODE/NAME มีอยู่ใน DB
...
TC-LOC13 ผ่านแล้ว → record NAME_UPDATED ยังคงมีอยู่ใน DB
...
TC-LOC07 ผ่านแล้ว → record มี name = NAME_UPDATED
...
## TCS-LOC09 — XSS payload ในชื่อต้องไม่รัน script
...
## TCS-LOC10 — SQL injection payload ต้องไม่ทำให้ระบบ crash
...
## TCS-LOC11 — ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100
...
## TCS-LOC12 — user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect _(skipped)_
