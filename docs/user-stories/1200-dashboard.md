...
_Authored from the test-case catalog `docs/test-cases/1200-dashboard.md` (documentation only — no automated spec yet)._
...
> **As an** authenticated user, **I want** the Dashboard to load and greet me, **so that** I have a confirmed, personalised entry point after logging in.
...
> **As an** authenticated user, **I want** the greeting to show my name, **so that** the dashboard feels personalised to me.
...
> **As an** authenticated user, **I want** the greeting to match the time of day, **so that** the dashboard feels timely and natural.
...
> **As an** authenticated user, **I want** to see today's date above the greeting, **so that** I can orient myself on each visit.
...
> **As an** authenticated user, **I want** my saved widgets to load, **so that** I see the metrics I pinned for myself.
...
> **As an** authenticated user, **I want** a count of my saved widgets, **so that** I can see at a glance how many are pinned.
...
> **As an** authenticated user, **I want** my widgets shown in my saved order, **so that** the most important ones appear first.
...
> **As an** authenticated user, **I want** scalar datasets shown as KPI cards, **so that** single-number metrics are easy to read.
...
> **As an** authenticated user, **I want** categorical datasets shown as pie charts, **so that** I can see the breakdown by category visually.
...
> **As an** authenticated user, **I want** a clear notice when a dataset shape can't be rendered, **so that** I'm not confused by a blank card.
...
> **As an** authenticated user, **I want** to pin a new widget from the dataset lookup, **so that** I can curate my own dashboard.
...
> **As an** authenticated user, **I want** to drag-reorder my widgets, **so that** my preferred arrangement persists across sessions.
...
> **As an** authenticated user, **I want** to remove a widget with confirmation, **so that** I can declutter my dashboard without accidental deletes.
...
> **As a** new authenticated user, **I want** guidance when I have no widgets yet, **so that** I know how to start building my dashboard.
...
> **As an** authenticated user, **I want** a graceful error message if my widgets fail to load, **so that** the page stays usable instead of crashing.
...
> **As an** unauthenticated visitor, **I want** to be redirected to login when I hit the dashboard, **so that** protected data stays protected.
