...
_Generated from `tests/159-pl.spec.ts` annotations. Edit annotations, not this file. Regenerate with `bun docs:user-stories`._

**Module:** Price List
**Spec:** `tests/159-pl.spec.ts`
**Default role:** Admin
**Total test cases:** 30 (11 High / 15 Medium / 4 Low)
...
| TC-PL-010050 | active BU = BLAVG | High | Smoke |
| TC-PL-010051 | หน้า list โหลดสำเร็จ (admin/BLAVG) + ปุ่ม Add แสดง | High | Smoke |
| TC-PL-010052 | ค้นหาคำที่ไม่มี → empty state | Medium | Negative |
...
> **As a** Admin user, **I want** this Price List behavior verified, **so that** the feature works as expected.
...
> **As a** Admin user, **I want** core Price List interactions to work, **so that** day-to-day usage stays smooth.
...
> **As a** Admin user, **I want** the Price List list page to load successfully, **so that** I can manage Price List records.
...
URL เป็น /vendor-management/price-list, heading 'Price List' แสดง, และปุ่ม Add แสดง (hard assert)
...
Login เป็น admin@blueledgers.com; active BU = BLAVG; มีสิทธิ์เข้าถึง Price List
...
<sub>Last regenerated: 2026-06-16 · git 51ce7d5</sub>
