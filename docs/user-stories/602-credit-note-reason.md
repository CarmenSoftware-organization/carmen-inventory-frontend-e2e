...
| --- | --- | --- | --- |
| TC-CNR00101 | หน้า list โหลดสำเร็จ | High | Smoke |
| TC-CNR00102 | ปุ่ม Add แสดง | High | Smoke |
| TC-CNR00103 | ช่องค้นหาใช้งานได้ | Medium | Smoke |
| TC-CNR00104 | ค้นหาคำที่ไม่มีต้องแสดง empty state | Medium | Functional |
| TC-CNR00105 | บันทึกโดยไม่กรอกชื่อต้องแสดง error | High | Validation |
| TC-CNR00106 | สร้างรายการใหม่และปรากฏในตาราง | High | CRUD |
| TC-CNR00107 | แก้ไขชื่อและบันทึก | High | CRUD |
| TC-CNR00108 | ลบรายการ | High | CRUD |
| TC-CNR00113 | แก้ไข: clear name แล้วบันทึก ต้องแสดง error | Medium | Validation |
| TCS-CNR00109 | XSS payload ในชื่อต้องไม่รัน script | High | Security |
| TCS-CNR00110 | SQL injection payload ต้องไม่ทำให้ระบบ crash | High | Security |
| TCS-CNR00111 | ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100 | Medium | Validation |
| TCS-CNR00112 _(skipped)_ | user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect | High | Authorization |

---

## TC-CNR00101 — หน้า list โหลดสำเร็จ

> **As a** Purchase user, **I want** the Credit Note Reason list page to load successfully, **so that** I can manage Credit Note Reason records.
...
> **As a** Purchase user, **I want** to see the Add button on the Credit Note Reason list, **so that** I can create new records.

**Priority:** High · **Test Type:** Smoke

**Preconditions**

Logged in as purchase@blueledgers.com; on /config/credit-note-reason
...
> **As a** Purchase user, **I want** to type into the Credit Note Reason search field, **so that** I can quickly locate existing records.
...
> **As a** Purchase user, **I want** a clear empty-state when no Credit Note Reason records match my search, **so that** I know nothing was found.
...
> **As a** Purchase user, **I want** the system to block invalid Credit Note Reason submissions, **so that** data quality is preserved.
...
> **As a** Purchase user, **I want** to create a new Credit Note Reason record, **so that** it becomes available for downstream operations.
...
> **As a** Purchase user, **I want** to edit an existing Credit Note Reason record, **so that** its data stays accurate.

**Priority:** High · **Test Type:** CRUD

**Preconditions**

TC-CNR00106 ผ่านแล้ว → record NAME มีอยู่ใน DB
...
> **As a** Purchase user, **I want** to delete a Credit Note Reason record, **so that** the list reflects only valid entries.

**Priority:** High · **Test Type:** CRUD

**Preconditions**

TC-CNR00113 ผ่านแล้ว → record NAME_UPDATED ยังคงมีอยู่ใน DB
...
TC-CNR00107 ผ่านแล้ว → record มี name = NAME_UPDATED
...
## TCS-CNR00109 — XSS payload ในชื่อต้องไม่รัน script
...
## TCS-CNR00110 — SQL injection payload ต้องไม่ทำให้ระบบ crash
...
## TCS-CNR00111 — ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100
...
## TCS-CNR00112 — user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect _(skipped)_
