...
| --- | --- | --- | --- |
| TC-CNR01 | หน้า list โหลดสำเร็จ | High | Smoke |
| TC-CNR02 | ปุ่ม Add แสดง | High | Smoke |
| TC-CNR03 | ช่องค้นหาใช้งานได้ | Medium | Smoke |
| TC-CNR04 | ค้นหาคำที่ไม่มีต้องแสดง empty state | Medium | Functional |
| TC-CNR05 | บันทึกโดยไม่กรอกชื่อต้องแสดง error | High | Validation |
| TC-CNR06 | สร้างรายการใหม่และปรากฏในตาราง | High | CRUD |
| TC-CNR07 | แก้ไขชื่อและบันทึก | High | CRUD |
| TC-CNR08 | ลบรายการ | High | CRUD |
| TC-CNR13 | แก้ไข: clear name แล้วบันทึก ต้องแสดง error | Medium | Validation |
| TCS-CNR09 | XSS payload ในชื่อต้องไม่รัน script | High | Security |
| TCS-CNR10 | SQL injection payload ต้องไม่ทำให้ระบบ crash | High | Security |
| TCS-CNR11 | ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100 | Medium | Validation |
| TCS-CNR12 _(skipped)_ | user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect | High | Authorization |
...
TC-CNR06 ผ่านแล้ว → record NAME มีอยู่ใน DB
...
TC-CNR13 ผ่านแล้ว → record NAME_UPDATED ยังคงมีอยู่ใน DB
...
TC-CNR07 ผ่านแล้ว → record มี name = NAME_UPDATED
...
## TCS-CNR09 — XSS payload ในชื่อต้องไม่รัน script
...
## TCS-CNR10 — SQL injection payload ต้องไม่ทำให้ระบบ crash
...
## TCS-CNR11 — ชื่อยาวเกิน maxLength ต้องถูกจำกัดที่ 100
...
## TCS-CNR12 — user สิทธิ์ต่ำเข้าหน้านี้ต้องไม่เห็นปุ่ม Add หรือถูก redirect _(skipped)_
...
<sub>Last regenerated: 2026-04-27 · git 64dc88b</sub>
