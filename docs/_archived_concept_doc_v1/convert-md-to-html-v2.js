...
// Enhanced HTML template with sidebar and TOC
const createHTMLTemplate = (title, content, breadcrumb, moduleInfo) => `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <aside class="sidebar">
        <div class="sidebar-header">
            <a href="${moduleInfo.backToModules}" class="back-link">← All Modules</a>
        ...
        <nav>
            <div class="nav-section">
                <h3>Navigation</h3>
                <ul>
                    <li><a href="${moduleInfo.moduleIndex}">Module Overview</a></li>
                    <li><a href="${moduleInfo.moduleIndex}#features">Features</a></li>
                </ul>
            </div>
        </nav>
    </aside>
...
// Function to determine module info based on file path
function getModuleInfo(filePath) {
    ...
    const moduleNames = {
        ...
        'cn': 'Credit Notes',
        'prt': 'Purchase Request Templates',
    ...
    // Calculate relative path to module index
    const depth = parts.length - 1;
    const upLevels = '../'.repeat(depth);

    return {
        moduleName: moduleNames[moduleCode] || moduleCode.toUpperCase(),
        moduleIndex: upLevels + 'index.html',
        backToModules: upLevels + 'index.html'
    };
}
...
function getBreadcrumb(filePath) {
    ...
    let breadcrumb = `<a href="${upLevels}index.html">Documentation</a>`;

    if (parts.length > 1) {
        const module = parts[0];
        breadcrumb += ` / <a href="${upLevels}index.html">${module.toUpperCase()}</a>`;
...
function convertFile(mdPath) {
    try {
        ...
        // Get breadcrumb and module info
        const breadcrumb = getBreadcrumb(mdPath);
        const moduleInfo = getModuleInfo(mdPath);

        // Generate HTML
        const html = createHTMLTemplate(title, mdContent, breadcrumb, moduleInfo);
...
console.log('Converting files with enhanced template...\n');
let converted = 0;
let failed = 0;
